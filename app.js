    ...
    for (i = rotated_files.length - 1; i >= RETAIN; i--) {
      (function (i) {
        ...
        if (
          ...
          const data = fs.readFileSync(path.resolve(dirName, rotated_files[i]))
...
function proceed(file) {
  ...
  var final_time = dayjs().format(DATE_FORMAT);
  // check for a timezone
  if (TZ) {
    try {
      dayjs.extend(timezone);
      final_time = dayjs().tz(TZ).format(DATE_FORMAT);
    } catch (err) {
      // use default
    }
  }
  var final_name = file.substring(0, file.length - 4) + '__' + final_time + '.log';
  // if compression is enabled, add gz extention and create a gzip instance
  if (COMPRESSION) {
    var GZIP = zlib.createGzip({ level: zlib.constants.Z_BEST_COMPRESSION, memLevel: zlib.constants.Z_BEST_COMPRESSION });
    final_name += ".gz";
  }
